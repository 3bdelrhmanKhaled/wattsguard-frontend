...
class Refrigerator:
    ...
    def get_unit(self, column):
        ...
        if 'capacity' in column_lower:
            return " L"
        elif 'electric power' in column_lower:
            return " kW"
        return ""
...
class PowerSummary:
    def __init__(self, searched_models, *databases):
        ...
        # Prioritize WashingMachine with EL1087567SLV
        for db in databases:
            db_name = db.__class__.__name__
            if db_name == "WashingMachine":
                for _, row in db.data.iterrows():
                    model = row.get("Model Name", "Unknown")
                    power = row.get("Electric Power", None)
                    if model == "EL1087567SLV" and power is not None and model in searched_models:
        ...
        for db in databases:
            ...
            for _, row in db.data.iterrows():
                model = row.get("Model Name", "Unknown")
                power = row.get("Electric Power", None)
                if model == "EL1087567SLV" and db_name == "WashingMachine":
                    continue
...
def distribute_power_consumption(total_consumption, power_df):
    ...
    excluded_categories = ["Wi-Fi Router", "Chargers & Small Devices", "Refrigerator"]
...
if __name__ == "__main__":
    try:
        ...
        electric_heater_db = ElectricHeater(FilePathConfig.ELECTRIC_HEATER_PATH)
        ...
        index = read_device_input(electric_heater_db, user_inputs, index)
        ...
        power_summary = PowerSummary(searched_models, device_db, electric_heater_db,
        ...
        raw_power_data = power_df.to_dict(orient="records")
        ...
        output = {
            ...
            "message": "Avoid using devices from 6 to 9 PM"
        }
